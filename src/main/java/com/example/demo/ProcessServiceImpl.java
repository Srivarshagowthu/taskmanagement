...
public class ProcessServiceImpl implements ProcessService {
  ...
  public ModelApiResponse createProcesses(List<Process> processes) throws CyborgException {
    ...
    List<Integer> existingProcessIds =
        existingProcesses.stream()
    ...
    for (Process process : processes) {
      ...
      if (existingProcessIds.contains(processId)) {
        ...
        throw new CyborgException(
            ErrorCode.valueOf("Process with ID " + processId + " already exists in the database."));
      }
      ...
      if (!uniqueIds.add(processId)) {
        ...
        throw new CyborgException(
            ErrorCode.valueOf("Process with ID " + processId + " is already in the request."));
      }
    }
    ...
    OffsetDateTime nowOffsetDateTime =
        nowInstant.atOffset(ZoneOffset.UTC); // Convert to OffsetDateTime
  ...
  public Page<Process> getAllProcesses(Integer page, Integer size) {
    ...
    return processRepository.findByDeletedNot(
        1, pageable); // Assuming this is the repository method
  }
  ...
  public ResponseEntity<ModelApiResponse> softDelete(Integer id) throws CyborgException {
    ...
    if (process == null) {
      ...
      return ResponseEntity.status(404)
          .body(new ModelApiResponse().message("Process not found").status("error"));
    }
    process.setDeleted(1); // Mark as deleted
    processRepository.save(process);
    log.info("Process with ID: {} soft deleted successfully", id);
    return ResponseEntity.ok(
        new ModelApiResponse().message("Process soft deleted successfully").status("success"));
  }
  ...
  public ResponseEntity<List<Process>> updateProcesses(List<Process> processes)
    ...
    List<Process> existingProcesses =
        processRepository.findAllById(
    ...
    for (Process existingProcess : existingProcesses) {
      ...
      existingProcess.setUpdatedAt(
          OffsetDateTime.ofInstant(new Date().toInstant(), ZoneOffset.UTC));
  ...
  private boolean validateProcess(Process process) {
    try {
      ...
      Set<String> excludedFields =
          new HashSet<>(
              Arrays.asList("updatedAt", "createdBy", "updatedBy", "deleted", "createdAt"));
